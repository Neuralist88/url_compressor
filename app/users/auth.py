...
def get_current_user(token: str = Depends(oauth2_scheme)) -> str:
    ...
    # Верифицируем токен и получаем его полезную нагрузку
    payload = verify_jwt_token(token)
    user_id = payload.get(
        "sub"
    )  # Предполагаем, что идентификатор пользователя находится в поле "sub"
    ...
    return user_id  # Возвращаем только user_id, так как нам нужен идентификатор пользователя
